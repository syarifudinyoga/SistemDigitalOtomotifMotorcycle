							...
							<form action="#" method="get">
								<table>
									<tr>
										...
										<td></td>
										<td>
												<input type="text" name="cari" id="cari" onKeyPress="return goodchars(event,'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ 0123456789',this)">
										</td>
										<td>
...
if(isset($_GET['cari'])){
	$cari = $_GET['cari'];
							...
							$product_array = $db_handle->runQuery("SELECT * FROM sukucadang where name like '%$cari%' ORDER BY NamaPerusahaan ASC");
} else {
	$product_array = $db_handle->runQuery("SELECT * FROM sukucadang ORDER BY NamaPerusahaan ASC");
}
							...
							?>
								...
								<form method="post" action="bengkel1.php?action=add&code=<?php echo $product_array[$key]["code"]; ?>">
									...
									<h6><?php echo $product_array[$key]["NamaPerusahaan"]; ?></h6>
